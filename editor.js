...
function pitchY(midi, baseY) {
  return baseY + 42 - (midi - 60) * 3;
}

function drawLedgerLines(x, y, staffY, group) {
  const top = staffY + 8, bottom = staffY + 40;
  if (y < top - 2) for (let yy = top - 8; yy >= y - 1; yy -= 8) svg('line', { x1:x-8, y1:yy, x2:x+8, y2:yy, class:'staff-line' }, group);
  if (y > bottom + 2) for (let yy = bottom + 8; yy <= y + 1; yy += 8) svg('line', { x1:x-8, y1:yy, x2:x+8, y2:yy, class:'staff-line' }, group);
}
...
function endpointFor(ref, layouts) {
  ...
  return { x: layout.x + 30 + (tick / MEASURE_TICKS) * (layout.width - 46), staffY:layout.staffY, tabY:layout.tabY, system:layout.system };
}
...
function drawAnnotations(layouts) {
  ...
  for (const ann of score.annotations) {
    ...
    if (ann.type === 'slur' || ann.type === 'tie') {
      ...
      const curve = (x1, x2, y) => svg('path', { d:`M${x1},${y} Q${(x1+x2)/2},${y-rise} ${x2},${y}`, class:ann.type });
      const drawOnSystem = (system, x1, x2) => {
        const bound = bounds.get(system);
        curve(x1, x2, bound.staffY + (ann.type === 'slur' ? 2 : 24));
        if (ann.type === 'slur' && score.instrument !== 'piano') curve(x1, x2, bound.tabY - 7);
      };
      if (a.system === b.system) drawOnSystem(a.system, a.x, b.x);
      else {
...
function render() {
  ...
  sys.forEach((items, systemIndex) => {
    ...
    const left = 36, usable = width - 55;
    const measureWidth = usable / items.length;
    svg('text', { x:2, y:staffY+28, text:'𝄞', 'font-size':42, 'font-family':'serif' });
    const piano = score.instrument === 'piano';
    svg('text', { x:piano ? 2 : 4, y:tabY+(piano?31:25), text:piano?'𝄢':'TAB', 'font-size':piano?38:11, 'font-family':'serif', 'font-weight':700 });
    ...
    items.forEach(({ measure, index }, localIndex) => {
      ...
      layouts.set(index, { x, width:measureWidth, staffY, tabY, system:systemIndex });
      ...
      for (let voice = 0; voice < measure.voices.length; voice++) {
        measure.voices[voice].forEach((note) => {
          ...
          const nx = x + 30 + (tick / MEASURE_TICKS) * (measureWidth - 46) + voice * 2;
...
function updateControls() {
  ...
  $('#dot').classList.toggle('active', dotted || !!selectedEntry()?.note.dotted);
...
function toMusicXml() {
  ...
  const measures = score.measures.map((m, mi) => {
    let body = `${mi > 0 && score.measures[mi-1].forceBreakAfter ? '<print new-system="yes"/>' : ''}${mi === 0 ? `<attributes><divisions>8</divisions><key><fifths>0</fifths></key><time><beats>4</beats><beat-type>4</beat-type></time><staves>2</staves><clef number="1"><sign>G</sign><line>2</line></clef>${score.instrument==='piano'?'<clef number="2"><sign>F</sign><line>4</line></clef>':'<clef number="2"><sign>TAB</sign><line>5</line></clef><staff-details number="2"><staff-lines>6</staff-lines></staff-details>'}</attributes>` : ''}`;
    m.voices.forEach((voice, vi) => {
      if (vi && voice.length) body += '<backup><duration>32</duration></backup>';
      voice.forEach((n) => {
        const pc = ((n.midi%12)+12)%12, octave = Math.floor(n.midi/12)-1;
...
function importMusicXml(text) {
  ...
  doc.querySelectorAll('part:first-of-type > measure').forEach((mx) => {
    ...
    mx.querySelectorAll(':scope > note').forEach((nx) => {
      ...
      note.midi = midi; note.rest = !!nx.querySelector('rest'); m.voices[voice].push(note);
    });
...
$('#dot').addEventListener('click', () => { const e=selectedEntry(); if(e){remember();e.note.dotted=!e.note.dotted;}else dotted=!dotted; updateControls();render(); });
...
export { score, toMusicXml, importMusicXml };
